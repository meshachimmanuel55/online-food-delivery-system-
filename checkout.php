...
if(empty($_SESSION["user_id"])){
    ...
    foreach ($_SESSION["cart_item"] as $item){
        ...
        if($_POST['submit']){
        $SQL="insert into users_orders(u_id,order_id,title,quantity,price) values('".$_SESSION["user_id"]."','".$order_id."','".$item["title"]."','".$item["quantity"]."','".$item["price"]."')";
        mysqli_query($db,$SQL);
        unset($_SESSION["cart_item"]);
        unset($item["title"]);
        unset($item["quantity"]);
        unset($item["price"]);
        $SQL1="insert into otp(u_id,order_id,otp,status) values('".$_SESSION["user_id"]."','".$order_id."','".$otp."','')";
